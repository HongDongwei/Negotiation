...
package com.example.negotiation.incall;
...
public class InCallActivity extends AppCompatActivity implements IOnCallActionTrigger,
        IOnLeftRightChoice, CallProximityManager.ProximityDirector, OnDtmfListener {
        ...
        */
        ...
        findViewById(R.id.bt_stop).setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                onTrigger(IOnCallActionTrigger.REJECT_CALL,getActiveCallInfo());
            }
        });
    }
    ...
    private void attachVideoPreview() {
        ...
        if(prefsWrapper.getPreferenceBooleanValue(SipConfigManager.USE_VIDEO)) {
            if(cameraPreview == null) {
                ...
                List<VideoUtilsWrapper.VideoCaptureDeviceInfo> infoList = VideoUtilsWrapper.getInstance().getVideoCaptureDevices(this);
                float realWidth = 0;
                float realHeight = 0;
                float pre_width = 360;
                float pre_height = 640;
                float size = pre_height / pre_width;
                if (null != infoList && infoList.size() > 0) {
                    VideoUtilsWrapper.VideoCaptureDeviceInfo deviceInfo = infoList.get(0);
                    pre_width = deviceInfo.bestCapability.height;
                    pre_height = deviceInfo.bestCapability.width;
                    //size = pre_height / pre_width;
                    Log.i(THIS_FILE, "size= " + size + " pre_width=" + pre_width + " pre_height" + pre_height);
                }
                realWidth = wm.getDefaultDisplay().getWidth();
                realHeight = size * realWidth;
                Log.i(THIS_FILE, "realHeight=" + realHeight + " realWidth=" + realWidth);


                RelativeLayout.LayoutParams lp = new RelativeLayout.LayoutParams((int) realWidth, (int) realHeight);
                //lp.leftMargin = 2;
                //lp.topMargin= 4;
